...
rf_Car <- randomForest(
	...
	y = parameter_list[, 2], 	# Response variable (Car values from the second column of parameter_list)
...
save(rf_Car, file=paste0(affix1,"/rf_Car.RData"))
...
rf.pred.Car <- predict(rf_Car, newdata=test_var)

# Create a data frame with the predicted Cab values and corresponding spectrum names
rf.pred.Car.df <- data.frame(
	Car = as.numeric(rf.pred.Car), 
	row.names = spectra_names
)

# Save the predictions to a file for later analysis or reporting
save(rf.pred.Car.df, file=paste0(affix1,"/predictions_Car.RData"))
